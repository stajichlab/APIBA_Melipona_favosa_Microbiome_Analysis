...
RUNNAME=${RUNNAME:-BEEHONEY_ITS_ASV_$(date +'%Y%m%d')}
BASE=$RUNNAME
...
if [ "$RUN_QIIME_CLASSIFIERS" = "1" ]; then
  ...
  if [ ! -s "$BASE.ASVs.uc.fa" ] || [ "$BASE.ASVs.fa" -nt "$BASE.ASVs.uc.fa" ]; then
    perl -pe 'tr/a-z/A-Z/' "$BASE.ASVs.fa" > "$BASE.ASVs.uc.fa"
  fi
  ...
  if [ ! -s "$BASE.ASVs.qza" ] || [ "$BASE.ASVs.uc.fa" -nt "$BASE.ASVs.qza" ]; then
    qiime tools import \
  ...
  ASV_SK_DIR="./$BASE.ASVs_taxonomy_bayesianclassifier"
  if [ ! -s "$ASV_SK_DIR/classification.qza" ]; then
    qiime feature-classifier classify-sklearn \
      --i-classifier "$UNITE_CLASSIFIER" \
      --i-reads "$BASE.ASVs.qza" \
      --output-dir "$ASV_SK_DIR"
  fi

  if [ ! -s "$ASV_SK_DIR/export/taxonomy.tsv" ]; then
    qiime tools export \
      --input-path "$ASV_SK_DIR/classification.qza" \
      --output-path "$ASV_SK_DIR/export"
  fi
  ...
  amptk taxonomy \
    -f "$BASE.ASVs.fa" \
    -i "$BASE.otu_table.txt" \
    -t "$ASV_SK_DIR/export/taxonomy.tsv" \
    -o "$BASE.ASVs.qiime_bayesian_taxonomy" \
    --cpus "$CPU" \
    --db ITS1
  module unload amptk
  ...
  amptk taxonomy \
    ...
    -o "$BASE.cluster.qiime_bayesian_taxonomy" \
    --cpus "$CPU" \
    --db ITS1
  module unload amptk

  if [ "$RUN_BLAST_CLASSIFIER" = "1" ]; then
    ASV_BL_DIR="./$BASE.ASVs_taxonomy_BLAST"
    if [ ! -s "$ASV_BL_DIR/classification.qza" ]; then
      qiime feature-classifier classify-consensus-blast \
        --i-query "$BASE.ASVs.qza" \
        --i-reference-taxonomy "$UNITE_TAXONOMY" \
        --i-reference-reads "$UNITE_SEQS" \
        --output-dir "$ASV_BL_DIR" \
        --p-perc-identity 0.80 \
        --p-maxaccepts 1 \
        --p-num-threads "$CPU"
    fi

    if [ ! -s "$ASV_BL_DIR/export/taxonomy.tsv" ]; then
      qiime tools export \
        --input-path "$ASV_BL_DIR/classification.qza" \
        --output-path "$ASV_BL_DIR/export"
    fi
    ...
    amptk taxonomy \
      -f "$BASE.ASVs.fa" \
      -i "$BASE.otu_table.txt" \
      -t "$ASV_BL_DIR/export/taxonomy.tsv" \
      -o "$BASE.ASVs.qiime_blast_taxonomy" \
      --cpus "$CPU" \
      --db ITS1
    module unload amptk
    ...
    amptk taxonomy \
      ...
      -o "$BASE.cluster.qiime_blast_taxonomy" \
      --cpus "$CPU" \
      --db ITS1
    module unload amptk
  fi
...
echo "Completed ITS ASV workflow in $BASE"
